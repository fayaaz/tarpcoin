 ...
 */
static const char* BITCOIN_PID_FILENAME = "TarpCoind.pid";
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/TarpCoin-project/TarpCoin>";
    const std::string URL_WEBSITE = "<https://TarpCoin.org>";
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if TarpCoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if TarpCoin is started while in a temporary directory.\n",
